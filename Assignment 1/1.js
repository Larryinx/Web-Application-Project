 ...
 * Date        : 2022.10.21 <fill in yourself>
 */
function special() {
    var targetDiv = document.getElementById("extra-bar");
...
function align() {
    var targetDiv = document.getElementById("posi");
    var targetDiv2 = document.getElementById("posi2");
...
function scrolll() {
    var targetDiv = document.getElementById("progress_bar");
    if (targetDiv.style.display == "none") {
        targetDiv.style.display = "block";
    }
    else {
...
function processform() {
    ...
    console.log(x);
    var inpObj = document.getElementById("new-email");
    ...
    var element = '<div><svg height="100" width="100"><circle cx="50" cy="50" r="40"></svg></div><div><h5></h5><p></p></div>';
    ...
    var lastComment = document.querySelector("#comments").lastElementChild; // instead of lastChild for div element
    ...
    // draw circle
    var color = document.querySelectorAll("input[name=new-color]:checked")[0].value; // look for checked radio buttons
...
function loadfile() {
    fetch('comments.txt')
        ...
        .then(function (txt) { return document.querySelector("#comments").innerHTML = txt; });
}
function savefile() {
    fetch('comments.txt', {
        method: 'PUT',
